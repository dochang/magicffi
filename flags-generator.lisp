...
output stream and input stream."
  (labels ((%extract-doc (cmt)
             (let* ((regex "/\\*(.*)\\*/")
                    (regs (nth-value 1 (cl-ppcre:scan-to-strings regex cmt))))
               (string-trim " " (aref regs 0))))
           (%get-lines ()
             (loop :for line := (read-line in nil)
                   :while line
                   :when (and (cl-ppcre:scan "#define" line)
                              (not (cl-ppcre:scan "_MAGIC_H" line)))
                     :collect (let* ((list (cl-ppcre:split "\\s+" line :limit 4))
                                     (name (second list))
                                     (cmt (fourth list))
